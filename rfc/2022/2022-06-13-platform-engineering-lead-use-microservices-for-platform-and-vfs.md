# RFC: use-microservices-for-platform-and-vfs
...
```mermaid
graph LR
    subgraph shared-infrastructure[Shared Infrastructure]
        public[Public] -->|TIC| aws(AWS)
        aws --> api-gateway{"platform-api.va.gov <br>(API gateway)"}
    end
    subgraph platform-services[Platform Services]
        api-gateway --> platform-vets-api["platform-vets-api <br> (legacy)"]
        api-gateway --> platform-sidekiq-ui
        api-gateway --> platform-feature-flags
        api-gateway --> platform-console-api
        api-gateway --> platform-file-upload
    end
    subgraph vfs-services[VFS Services]
        api-gateway --> vfs-app-a.vfs.va.gov
        api-gateway --> vfs-app-b.vfs.va.gov
        api-gateway --> vfs-app-c.vfs.va.gov
    end
```
