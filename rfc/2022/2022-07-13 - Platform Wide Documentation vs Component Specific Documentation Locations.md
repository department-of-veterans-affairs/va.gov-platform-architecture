...
1. **A self-service process for adding documentation.** Teams who own software components, services, and resources will register these entities in the Console catalog. They will include references to their technical documentation and spell out both upstream and downstream dependencies. They may also create catalog entries for dependencies (and their documentation) that are external to the VA.
    1. Most teams would create a ```/docs``` folder at the root of their project to contain markdown files and subfolders for organizing their documentation.
    ...
    1. Documentation for TechDocs can be found here: (TechDocs "HOW TO" guides)[https://backstage.io/docs/features/techdocs/how-to-guides]. It is possible to store TechDocs outside of the source code repository, and we have considered that there could be benefits to using GitHub Wiki repositories to make editing documentation more convenient. Here are some alternatives in the event that you want to store documents somewhere other than in the root of your repository:
        1. A team can specify that their documentation lives in a subfolder, "dir:./sub-folder". So a VFS team with an app in the vets-website repo can reproduce the same ```docs``` folder, ```mkdocs.yml``` manifest, and the ```docs/index.md``` initial help file where their source code is located.
        1. Another option is, "you can instead specify a URL location reference, the exact value of which will vary based on the source code hosting provider." This option has a different start to the annotation. Rather than ```dir:``` it would use ```url:``` and look something like this: ```url:https://github.com/backstage/backstage/tree/master/plugins/techdocs-backend/examples/documented-component```. So developers could use this to refer to component documentation at a direct URL in the repository, just with a different protocol/method for retrieving the documentation. Developers can also use this option to point to the GitHub Wiki repository. That approach might help give people a richer editing interface and another way to approach the documentation.
